...
|===
| ID#      ^| Requirement
| `SEC_010`  | MUST implement a hardware root of trust (RoT).
2+| _A root of trust (RoT) is the foundation on which all secure operations of a
     ...
     A RoT provides security services such as:
     * Boot and attestation.
     * Security lifecycle management.
     * Key derivation.

    A HW RoT moves critical functions and assets off an application processor
    Hart to a dedicated and possibly isolated trusted subsystem, which can provide
    stronger protection against physical and logical attacks._

| `SEC_020`  | MUST manage a security life cycle.
2+| _A security life cycle reflects the trustworthiness of a system during its
     lifetime and reflects the life cycle state of hardware provisioned assets.
     The minimum security life cycle includes the following states:
     ...
     * Secured - hardware provisioned assets are locked (immutable), only
       authorized software can be used, and revealing debug is not enabled.
     * Recoverable debug - part of the system is in a revealing debug state.
       At least the RoT is not compromised and hardware provisioned secrets
       remain protected.
     * Terminated - Hardware provisioned assets are made permanently inaccessible
       and revoked before entering this state. This includes derived assets such
       as attestation keys._

| `SEC_030`  | MUST implement a secure identity and must support platform attestation.
2+| _A secure identity is an element capable of generating a cryptographic
     signature which can be verified by a remote party. The secure identity
     ...
     completion of security provisioning.

     Platform attestation allows a remote relying party to determine the
     trustworthiness of the platform before submitting assets to it. The
     platform attestation must be signed by a hardware RoT using a hardware
     provisioned secure identity._

| `SEC_040`  | MUST implement UEFI Secure Boot and Driver Signing (cite:[UEFI] Section 32)
| `SEC_050`  | It MUST be possible for a physically present user to disable Secure Boot enforcement, thus allowing unsigned code to be executed.
| `SEC_060`  | It MUST be possible for a physically present user to fully manage the contents of all Secure Boot key stores (PK, KEK, db and dbx). This includes the ability to delete all factory-provided keys, enrolling their own custom keys, and resetting all key stores to their factory state.
| `SEC_070`  | MUST back the UEFI Authenticated Variables implementation with
             a mechanism that cannot be accessed or tampered by an unauthorized
             software or hardware agent.
| `SEC_080`  | MUST implement in-band firmare updates as per cite:[BRS].
...
| `SEC_110`  | It MUST not be possible to bypass secure boot, authentication or digital signature failures.
|===
