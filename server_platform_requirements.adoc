...
2+| _Many of these mandated extensions are optional in the RVA23 ISA profile.
     These mandates may be moved into a new ISA profile specification._
...
2+| _This is a stronger requirement than the Server SoC `MNG_030` rule cite:[ServerSoC]. This specification does not provide guidance around how the UART is physically exposed, i.e. via RS232 signalling, USB, a BMC or other mechanism._
