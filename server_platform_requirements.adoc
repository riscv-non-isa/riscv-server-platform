...
| `SEC_011`  | It MUST be possible for authorized users to disable Secure Boot enforcement, thus allowing unsigned code to be executed.
2+| _The determination of the definition of an authorized user is left to the
    platform integrator and customer._
| `SEC_012`  | It MUST be possible for authorized users to fully manage the contents of all Secure Boot key stores (PK, KEK, db and dbx). This includes the ability to delete all factory-provided keys, enrolling their own custom keys, and resetting all key stores to their factory state.
...
| `SEC_060`  | It MUST not be possible for unauthorized users to bypass secure boot, authentication or digital signature failures.
|===
