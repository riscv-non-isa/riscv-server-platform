...
| `SEE_020` a| The RISC-V application processor hart MUST support:
             ...
             * All triggers MUST support
               ...
               ** Filtering using mhselect values of 0, 2, and 6.
               ** Filtering using sselect values of 0 and 2.

             * Triggers MAY support

               ** Filtering using mhselect values of 1 and 5.
               ** Filtering using sselect values of 1.

             * Supported mhselect and sselect values MUST be identical for all triggers.
...
| `FIRM_140` | IOMMU MUST be described using the RIMT ACPI table cite:[RIMT].
...
2+| _A Root of Trust (RoT) is a component that performs one or more
     ...
     provides stronger protection against both physical and logical attacks.

     Examples of open-source RoT IPs include OpenTitan (cite:[OT]) and
     Caliptra (cite:[CLP])._
...
2+| _The term "locked down" refers to the (optional) ability to prevent the
    ...
    security lifecycle state has been reached. This could be implemented, for
    example, via an eFuse.
...
2+| _Cryptographic algorithms and key sizes used for firmware update signing and
    verification should comply with the security and regulatory requirements of the
    geographies or markets in which the platform operates. Examples of relevant
    standards include the U.S. National Institute of Standards and Technology
    (NIST) cryptographic guidelines and the Commercial National Security Algorithm
    (CNSA) Suite._
