...
| `FIRM_120` | If the platform implements forward-edge control-flow integrity, the runtime services must be compiled to support it.
2+| _The platform SHOULD implement the SBI FWFT LANDING_PAD interface._
| `FIRM_130` | The support for forward-edge control-flow integrity in runtime services MUST be sigaled by the EFI_MEMORY_ATTRIBUTES_FLAGS_RT_FORWARD_CONTROL_FLOW_GUARD flag (cite:[UEFI] Section 4.6.3 EFI_MEMORY_ATTRIBUTES_TABLE).
|===
